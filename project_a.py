...
def visualize_model(confidence, preds, inputs, labels, class_names, num_images=4, picpath='.'):
    images_so_far = 0
    fig = plt.figure()
    ...
    for j in range(inputs.size()[0]):
        ...
        ax.axis('off')
        ax.set_title(f'Label: {class_names[labels[j]]}, Pred: {class_names[preds[j]]}, Conf: {confidence[j]}')
        imshow(inputs.cpu().data[j])
        plt.pause(0.001)

        if images_so_far == num_images:
            picpath = os.path.join(picpath, 'output.jpg')
...
def test_model(model, dataloader, criterion, class_names, dataset_size, check_minibatch=10, device='cuda'):
    ...
    for inputs, labels in dataloader:
        ...
        if check_idx % check_minibatch == 0:
            visualize_model(confidence, preds, inputs, labels, class_names, num_images=4)
